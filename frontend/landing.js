...
const animateCounter = (element, target, duration = 2000) => {
    ...
    let start = 0;
    const increment = cleanTarget / (duration / 16); // 60fps
    ...
    const counter = setInterval(() => {
        ...
        if (target.includes('K')) {
            ...
            displayValue = '<' + Math.ceil(start) + 's';
        }

        if (hasPlus && start >= cleanTarget) {
            displayValue += '+';
        }
...
const updateGreeting = () => {
    ...
    if (hour < 12) {
        // Morning - keep original
    } else if (hour < 18) {
        // Afternoon
        heroTitle.innerHTML = heroTitle.innerHTML.replace('Growing Together', 'Growing Smarter');
    } else {
        // Evening
        heroTitle.innerHTML = heroTitle.innerHTML.replace('Growing Together', 'Growing Sustainably');
    }
};
...
console.log('%c🌱 Digital Krishi - AI Plant Identifier', 'color: #2d7a3e; font-size: 20px; font-weight: bold;');
console.log('%cBuilt with ❤️ for farmers everywhere', 'color: #4caf50; font-size: 14px;');
